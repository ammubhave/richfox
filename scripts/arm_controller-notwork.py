...
class arm_controller():
  ...
  def __init__(self, arm='l'):
    self.GD = controller_manager.ControllerManager(arm, using_slip_controller=False, using_slip_detection = False)
  ...
  def move_rel_cartesian_step(self, vector):
  ...
  def move_cartesian_step(self, pose, timeout = 3.0,
...
class left_arm_controller(arm_controller):
  def __init__(self):
    arm_controller.__init__(self, 'l')

class right_arm_controller(arm_controller):
  def __init__(self):
    super(right_arm_controller, self).__init__('r')
...
if __name__=="__main__":
  ...
  ctrl.move_rel_cartesian_step([0,1,0])
  raw_input("quit?")
